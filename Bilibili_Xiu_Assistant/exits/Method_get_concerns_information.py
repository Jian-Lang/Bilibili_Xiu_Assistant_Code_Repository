...
def get_concerns_information(vmid_list):
    ...
    if len(vmid_list) != 0:
        ...
        for n in range(0, len(vmid_list)):
            message_search_api = 'http://api.bilibili.com/x/space/acc/info?mid=%s' % vmid_list[n]
            response2 = requests.get(message_search_api, proxies = proxy)
